...
class GenPageBrowserClass {
    ...
    buildContentList(container, files) {
        ...
        for (let file of files) {
            ...
            if ((file.name.endsWith('.mp4') || file.name.endsWith('.webm')) && id < 10) {
                img = document.createElement('video');
                img.setAttribute('controls', 'true');
                img.setAttribute('loop', 'false');
                img.setAttribute('autoplay', 'false');
                img.pause();
                if (this.format.includes('Thumbnails')) {
                    img.setAttribute('style', 'max-height: 100px')
                    if (this.format.startsWith('Big')) { img.setAttribute('style', 'max-height: 200px') }
                    else if (this.format.startsWith('Giant')) { img.setAttribute('style', 'max-height: 250px') }
                    else if (this.format.startsWith('Small')) { img.setAttribute('style', 'max-height: 60px') }
                } else if (this.format == 'List') {
                    img.setAttribute('style', 'max-height: 50px')
                } else if (this.format.includes('Cards')) {
                    img.setAttribute('style', 'max-height: 100px')
                    if (this.format.startsWith('Small')) { img.setAttribute('style', 'max-height: 100px') }
                    else if (this.format.startsWith('Big')) { img.setAttribute('style', 'max-height: 200px') }
                }
